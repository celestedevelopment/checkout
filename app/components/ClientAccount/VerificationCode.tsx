...
export default function VerificationCode({
  ...
  return (
    <div className="transition-all duration-300 ease-in-out">
      ...
      <div className="mb-3">
        <h3 className="text-base font-medium text-gray-900 mb-2 select-none">{t('enterVerificationCode', 'Enter your verification code')}</h3>
        <p className="text-xs text-gray-600 mb-3 select-none">
          {t('verificationCodeInstructions', 'Enter the code sent to')} <span className="font-medium text-gray-900 select-none">{email}</span> {t('toSecurelyAuthenticate', 'to securely authenticate your account')}.
        </p>
          ...
          <div className="mb-3 p-2 bg-red-50 border border-red-200 rounded-md">
            <div className="flex items-center gap-2">
              ...
              <span className="text-red-700 text-xs font-medium select-none">
                {codeError || t('verificationCodeInvalid', 'Verification code was invalid')}
              </span>
            </div>
          </div>
        )}
        ...
        {resendSuccess && (
          <div className="mb-3 p-2 bg-green-50 border border-green-200 rounded-md">
            <div className="flex items-center gap-2">
              ...
              <span className="text-green-700 text-xs font-medium select-none">{t('verificationCodeResent', 'Verification code has been resent to your email')}.</span>
            </div>
          </div>
        )}
        ...
        <div className="text-xs text-gray-600 mb-2 select-none">
          ...
          >
            {isResending
              ? t('sending', 'Sending...')
              : resendCooldown > 0
              ? t('resendCodeCooldown', `Resend code (${resendCooldown}s)`)
